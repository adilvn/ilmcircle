...
import "../../../StudentDashboard/SidebarContent/Chat/messages.css";
...
import { ContactOffCanvas } from "./OrgContactOffCanvas";
import secureLocalStorage from "react-secure-storage";
import API_Routes, { url } from "../../../Routes/API_Routes";
import { showMessage } from "../../../components/reuseable/Tostify";
import { Loader } from "../../../components/reuseable/Loader";
import OrgContactDetails from "./OrgContactDetails";
import { OrganizationContact } from "../../../StudentDashboard/SidebarContent/Contact/OrganizationContact";
import OrganizationContactOffcanvas from "../../../StudentDashboard/SidebarContent/Contact/OrganizationContactOffcanvas";
...
function Chat(props) {
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      const res = await fetch(`${url}api/organization/allusers`, {
        method: "GET",
        headers: myHeaders,
      });
      const resData = await res.json();
      if (resData.status == 200) {
        // console.log(resData.data, "<<<<<<<<<<<<<>>>>>>>>>>>>>STudent Contact");
        // console.log(resData?.data.organization  ,"lastName")
        setLoader(false);
  ...
  const blockUnBlockUser = async () => {
    ...
    fetch(`${url}api/organization/block`, requestOptions)
  ...
  const sendMessage = async () => {
    ...
    fetch(`${url}api/organization/message/create`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
  ...
  const fetchBuddy = async (item, index) => {
    ...
    try {
      ...
      myHeaders.append("Authorization", "Bearer " + token);
      const res = await fetch(`${url}api/organization/buddy/${item._id}`, {
        method: "GET",
        headers: myHeaders,
      });
      const resData = await res.json();
                      ...
                      {data1?.organization?.length > 0 && (
                        <>
                          <div class="accordion-item">
                            ...
                            >
                              <div class="accordion-body px-2">
                                {data1?.organization?.map((item, index) => {
                                  return (
                                    <div key={index}>
                                      {console.log(item)}
                                      <div className="message">
                                          ...
                                          <div>
                                            <div
                                              ...
                                              onClick={() => {
                                                ...
                                                // fetchBuddy(item, index)
                                                // showMessage(' The screen is currently under development')
                                              }
                                            >
                                              <div className="d-flex ">
                                                <img
              ...
              {!canvasScreen ? (
                ...
                >
                  {orgActiveIndex != null && (
                    ...
                    >
                      <OrganizationContact data={organizationData} data2={organizationData2} />
                    </div>
                  )}
                </div>
              ) : (
                <OrganizationContactOffcanvas data={organizationData} data2={organizationData2} />
              )}
            </>
          )}
        </div>
