...
import "../../OrganizationDashboard/OrgSidebarContent/Notification/Notification.css";
...
export const StuNotifications = () => {
    ...
    // Initialize the reply state as an array

    // Function to update the reply text for a specific item
    ...
    const GetNotifications = () => {
        ...
        fetch(API_Routes.GETNOTIFICATION, requestOptions)
    ...
    const UpdateNotification = (e, item, sendAll) => {
        ...
        fetch(API_Routes.UPDATENOTIFICATION, requestOptions)
            .then(response => response.json())
            .then(result => {
                if (result.status == 200 || result.status == 201) {
                    GetNotifications()
    ...
    const UpdateFeedbackData = (item, data, action, replyEntry) => {
        setLoader(true)
        var myHeaders = new Headers();
        myHeaders.append("Authorization", "Bearer " + token);

        var formdata = new FormData();
        formdata.append("id", replyEntry?.itemId);
        if (data == 0) {
    ...
    const handleDelete = (item, delAll) => {
        ...
        fetch("https://ilmcircle.com/backend/api/student/notification/delete", requestOptions)
            .then(response => response.json())
            .then(result => {
                if (result?.status == 200 || result?.status == 201) {
                    setLoader(false)
    ...
    const sendMessage = (message, text) => {
        ...
        const chatID = JSON.parse(message?.details?.chatId);
        const receiverId = JSON.parse(message?.details?.userId);
        var myHeaders = new Headers();
        myHeaders.append("Authorization", "Bearer " + token);
        ...
        fetch("https://ilmcircle.com/backend/api/student/message/send", requestOptions)
            .then((response) => response.json())
            .then((result) => {
                if (result?.status == 200 || result?.status == 201) {
                    setLoader(false);
        ...
        <div className="notificationMain ">
            ...
            <div className="p-md-5 p-3 m-md-5">
                ...
                <div className="section mt-5 pt-3">
                            ...
                            <p className="mb-0 ms-2">
                                {filteredEventData?.every((item) => item.selected) ? (
                                    <div>
                                        ...
                                        <a onClick={(e) => UpdateNotification(e, "", 4)} className="orangeColor allOrangelinks ms-3">Mark as unread</a>
                                        <a onClick={() => handleDelete("", true)} className="orangeColor allOrangelinks ms-3">Delete Selected Notification</a>
                                    </div>
                                ) : (
                                    "Select all"
                                )}
                    ...
                    {filteredEventData?.length ? filteredEventData?.map((item) => {
                        const inputVal = reply?.find((entry) => entry.itemId === item._id);
                        return (
                            <div key={item._id} className={item.isRead ? "item2" : "item"}>
                                <div onClick={() => handleClickRow(item)} className="d-flex justify-content-between">
                                            ...
                                            {JSON.parse(item?.details?.index) === 5 && item.isReply === false && (
                                                <div className="position-relative mt-3">
                                                    ...
                                                    {!inputVal?.text ? (
                                                        ...
                                                        <BsSend
                                                            onClick={() => sendMessage(item, (reply.find((entry) => entry.itemId === item._id) || {}).text)}
